...
class engine
{
	...
	var $height;
	var $max_items;
	var $pattern='/<\s*img [^\>]*src\s*=\s*[\""\']?([^\""\'\s>]*)/i';	// img pattern

	// options
	var $wpb_div_banner_style;
	var $wpb_tooltip_story_lenght;
	var $wpb_div_label_style;
	var $wpb_div_text_style;
	var $wpb_text_style;
	...
	function add_item($i)
	{
		...
		if($this->VALID_CONFIGURATION==True)
		{
			if(sizeof($items)<$this->max_items)
			{
				...
				preg_match_all($this->pattern,$i->img,$m);
				$found_images=sizeof($m[1]);
				for($k=0;$k<$found_images;$k++)
				{
					$img=trim($m[1][$k]);
					engine::calc_image_dimensions($img,$this->width,$this->height,$nw,$nh);
	...
	function render()
	{
		...
		if($this->VALID_CONFIGURATION==True)
		{
			if($items<>NULL)
			{
				...
				$html="<SCRIPT SRC='" . engine::script_path(). "/scripts/boxover.js'></SCRIPT>\n";
				...
				foreach($items as $item)
				{
					...
					$html.= engine::resize_img($item->img,$this->width,$item->calculated_height,$item->crop);
	...
	function render_slideshow()
	{
		...
		if($this->VALID_CONFIGURATION==True)
		{
			if($items<>NULL)
			{
				...
				$html ="<script src=\"" . engine::script_path(). "/scripts/mootools.v1.11.js\" type=\"text/javascript\"></script>\n";
				$html.="<script src=\"" . engine::script_path(). "/scripts/jd.gallery.js\" type=\"text/javascript\"></script>\n";
				$html.="<link rel=\"stylesheet\" href=\"" . engine::script_path(). "/css/jd.gallery.css\" type=\"text/css\" media=\"screen\" />\n";
				...
				foreach($items as $item)
				{
					...
					$html.=engine::resize_img($item->img,$this->width,$item->calculated_height,$item->crop,'class="full"');
					$html.="<img src=\"" . engine::script_path(). "/resize.php?src=$item->img&w=75&h=75&crop=75\" class=\"thumbnail\" />";
					$html.="</div>\n";
	...
	{
		$code = "<img src='" . engine::script_path() . "/resize.php?";
